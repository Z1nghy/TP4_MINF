...
void ScanPec12 (bool ValA, bool ValB, bool ValPB, bool ValS9)
{
   ...
   {
        if(Pec12.PressDuration < TEMPS_ESC)
        { 
            Pec12.PressDuration++;
        }
   ...
   if(DebounceGetInput(&DescrS9) == 0) 
   {
        if(S9.PressDuration < TEMPS_ESC)
        { 
            S9.PressDuration++;
        }
   ...
   if(DebounceIsReleased(&DescrPB))
   {
       ...
       if(Pec12.PressDuration > 0) 
       {
           if(Pec12.PressDuration >= TEMPS_ESC)
           {
               Pec12.ESC = 1; // Bouton presse plus de 500ms
               Pec12.OK = 0;
           } 
           else
           {
               Pec12.ESC = 0;
               Pec12.OK = 1;            // Bouton presse moins de 500ms
           }
       }
   ...
   if(DebounceIsReleased(&DescrS9))
   {
       ...
       if(S9.PressDuration > 0) 
       {
           if(S9.PressDuration >= TEMPS_ESC)
           {
               S9.ESC = 1; // Bouton presse plus de 500ms
               S9.OK = 0;
           } 
           else
           {
               S9.ESC = 0;
               S9.OK = 1;            // Bouton presse moins de 500ms
           }
       }
       S9.PressDuration = 0; 
   } 

    // Gestion inactivité PEC12
    if(Pec12.InactivityDuration < INACTIVITY_DELAY)
    {
    ...
    // Gestion inactivité S9
    if(S9.InactivityDuration < INACTIVITY_DELAY)
    {
...
void Pec12Init (void)
{
   ...
   // Init de la structure PEc12
    ...
    S9.PressDuration = 0; // Pour durée pression de S9
