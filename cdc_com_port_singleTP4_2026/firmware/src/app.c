...
void APP_Tasks (void )
{
    ...
    appData.usbStat = appData.isConfigured;
    //int i; 
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_SWITCH_PRESSED:
            ...
            /* On attend que la lecture soit physiquement terminée */
            if(appData.isReadComplete == true)
            {
                // On réarme le flag pour la prochaine lecture
                //appData.isReadComplete = false; 
                
                appData.state = APP_STATE_SCHEDULE_WRITE;

                // SÉCURITÉ : On vérifie que le PC a bien envoyé des données (> 0)
                if (appData.numBytesRead > 0) 
                {
                    bool saveTodo = false;
                    bool isSaved = false;

                    // SÉCURITÉ : Anti-débordement du buffer
                    if (appData.numBytesRead < APP_READ_BUFFER_SIZE) {
                        appData.readBuffer[appData.numBytesRead] = '\0';
                    } else {
                        appData.readBuffer[APP_READ_BUFFER_SIZE - 1] = '\0';
                    }
                    
                    // 1. On lit et on décode le message reçu
                    if (GetMessage((int8_t*)appData.readBuffer, &RemoteParamGen, &saveTodo))
                    {
                        if (saveTodo)
                        {
                            MENU_DemandeSave();
                        ...
                        // Si tout est bon, SendMessage gère l'envoi de la confirmation
                        SendMessage((int8_t*)appData.readBuffer, &RemoteParamGen, isSaved);
                    }
                    else 
                    {
                        // SI LE MESSAGE EST FAUX : On l'ignore !
                        // On retourne simplement écouter le port USB sans faire crasher le C#
                        appData.state = APP_STATE_SCHEDULE_WRITE;
                    }
                }
                else 
                {
                    // Si on reçoit un paquet vide (Zero Length Packet), on l'ignore
                    //appData.state = APP_STATE_SCHEDULE_WRITE;
                }
            }
...
//            RemoteParamGen = LocalParamGen;
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            //}
            //bool saveTodo = false;
            //if (GetMessage((int8_t*)appData.readBuffer, &LocalParamGen, &saveTodo)) 
            //&&{
...
//                    APP_GEN_setSaveRequested(saveRequested);
                ...
                //SendMessage((int8_t*)appData.readBuffer,&RemoteParamGen, appData.isSwitchPressed);
            ...
            // J'ai SUPPRIMÉ le SendMessage() qui était ici pour éviter les conflits !
            break;
